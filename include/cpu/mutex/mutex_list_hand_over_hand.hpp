...
// a set ADT. Same abstraction and invariants as the coarse-grained mutex::List,
// but the single list mutex is replaced by a per-node std::mutex, and traversal
// couples locks so that concurrent operations on disjoint regions proceed in
// parallel (Bayer & Schkolnick, 1977).
//
// Invariants:
...
private:
  ...
  struct Window {
    Node *pred;
    Node *curr;
    std::unique_lock<std::mutex> predLock;
...
private:
  ...
  Window search(const T &key) {
    ...
    std::unique_lock<std::mutex> predLock(pred->mutex);

    Node *curr = pred->next;
    std::unique_lock<std::mutex> currLock(curr->mutex);
    ...
    while (curr != &m_tailSentinel && curr->key < key) {
      ...
      pred = curr;
      curr = curr->next;
      currLock = std::unique_lock<std::mutex>(curr->mutex);
    }
